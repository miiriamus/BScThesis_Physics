# -*- coding: utf-8 -*-
"""
Created on Fri Jun  5 14:00:19 2026

@author: Miriam_Ucendo
"""
"""
kuramoto_lorentzian_multiN_MF.py
===================
Kuramoto model with Lorentzian frequency distribution — mean-field (all-to-all) case.
...
seed           = 130402
export_png     = True
png_filename   = f"MF_multiN_d{Delta}_T{T}.png"
...
COLORS = [cm.plasma(i / max(len(N_LIST) - 1, 1)) for i in range(len(N_LIST))]
# ──────────────────────────────────────────────
...
# ── Single-N sweep ────────────────────────────
def run_sweep_for_N(N, nn, Delta, omega0, K_points, T, dt, transient_frac, seed, K_array):
    ...
    is_all_to_all  = np.array_equal(A, all_to_all_ref)
...
if __name__ == "__main__":
    ...
    results = {}
    for N in N_LIST:
        nn = (N+1) // 2         # mean-field: cada oscilador conectado a todos los demás
        t0 = time.time()
        print(f"\nRunning N={N}, nn={nn} (all-to-all) ...")
        r = run_sweep_for_N(N, nn, Delta, omega0,
                            K_points, T, dt, transient_frac, seed, K_array)
        results[N] = r
        Kc_n = detect_kc_numerical(K_array, r)
        f_list.append( 100* 2*nn / (N - 1) )
        kc_str = f"{Kc_n:.3f}" if Kc_n is not None else "not detected"
        print(f"  N={N:>4d} | Kc_numerical={kc_str} | elapsed={time.time()-t0:.1f}s")
    ...
    ax1.set_title(
        fr"Kuramoto model — Lorentzian ($\Delta={Delta}$, $\omega_0={omega0}$, mean-field)",
        pad=8, fontsize=10.5
    )
